      ...
      <main className="min-h-screen bg-white text-black">
        <div className="max-w-6xl mx-auto px-6 py-12">
          <nav className="text-xs text-gray-500 mb-6" aria-label="Breadcrumb">
            ...
            {' / '}
            <span className="text-[#003D5C] font-medium">Cities</span>
          </nav>
          ...
          <p className="text-lg text-gray-700 mb-10 max-w-3xl leading-relaxed">
            ...
            <Link href="/state" className="underline hover:text-[#003D5C]">state</Link>
            {' '}or{' '}
            <Link href="/specialty" className="underline hover:text-[#003D5C]">specialty</Link>.
          </p>
                  ...
                  <ul className="flex flex-wrap gap-2">
                    {hubs.map((hub) => (
                      <li key={hub.slug}>
                        <Link
                          href={`/city/${hub.slug}`}
                          className="inline-block rounded-md border border-gray-200 shadow-sm px-3 py-1.5 text-sm font-bold hover:bg-[#003D5C] hover:text-white transition-colors"
                        >
                          {hub.name.split(',')[0]} →
                        </Link>
                      </li>
